// 遊戲設定與資料表 ------------------------------------------------

export const START_COINS = 100000;

// 每次命中「擊殺」的機率（魚機式捕獲率）。約 5%，敵將更難擊殺。
export const KILL_CHANCE = 0.05;
export const BOSS_KILL_FACTOR = 0.35; // 敵將擊殺機率 = KILL_CHANCE × 此值

// 下注階梯
export const BET_LEVELS = [10, 20, 50, 100, 200, 500, 1000];

// 玩家可切換的武將砲台
export const GENERALS = [
  { name: '關羽', color: 0x2fae4a, robe: 0x1f7a33, blade: 0x7fe0a0 }, // 蜀・綠
  { name: '張飛', color: 0x3a4a6a, robe: 0x22304d, blade: 0x9ab0e0 },
  { name: '趙雲', color: 0xd8d8e0, robe: 0xb0b6c8, blade: 0xffffff },
];

// 左右兩側的 AI 弓箭手座位（會自動瞄準敵人拉弓放箭，各自累積籌碼）
// generalIndex 對應 GENERALS，x 為弓箭手的世界座標橫向位置
// drawTime = 拉滿弓所需秒數，recoverTime = 放箭後回復 / 搭新箭秒數
export const AI_PLAYERS = [
  { seat: 'left',  name: '常山趙子龍', generalIndex: 1, x: -14, coins: 5_284_900, betIndex: 4, drawTime: 0.55, recoverTime: 0.4 },
  { seat: 'right', name: '江東小霸王', generalIndex: 2, x: 14,  coins: 9_130_500, betIndex: 5, drawTime: 0.45, recoverTime: 0.3 },
];

// 選房用假資料：20 個房間，每房三個座位（左/中/右），各房分配不同的人。
// 每房固定留一個空位（name: null）供玩家入座，對應選房頁面的佔用感。
// seat.pos: 'left' | 'mid' | 'right'，generalIndex 對應 GENERALS。
const ROOM_NAME_POOL = [
  '常山趙子龍', '燕人張翼德', '臥龍先生', '江東小霸王',
  '虎痴許褚', '錦帆賊甘寧', '美髯公', '飛將軍',
  '錦馬超', '小李廣', '奉先無雙', '鳳雛龐統',
  '虎威將軍', '西涼錦兒', '江東周郎', '常勝將軍',
];

function buildRooms() {
  const rooms = [];
  for (let i = 0; i < 20; i++) {
    const num = 41 + i;                 // 休閒41房 ~ 休閒60房
    const emptyPos = (i + 1) % 3;       // 每房輪流留一個空位；房41(i=0)為中座，玩家預設坐中央
    const positions = ['left', 'mid', 'right'];
    const seats = positions.map((pos, s) => {
      const isEmpty = s === emptyPos;
      const nameIdx = (i * 3 + s) % ROOM_NAME_POOL.length;
      return {
        pos,
        name: isEmpty ? null : ROOM_NAME_POOL[nameIdx],
        generalIndex: (i + s) % GENERALS.length,
        coins: isEmpty ? 0 : (500_000 + ((i * 7 + s * 3) % 90) * 123_400),
        betIndex: (i + s) % BET_LEVELS.length,
      };
    });
    rooms.push({ id: num, name: `休閒${num}房`, seats });
  }
  return rooms;
}

export const ROOMS = buildRooms();
export const ROOMS_PER_PAGE = 10;

// 三個座位在戰場上的橫向世界座標（左 / 中 / 右）
export const SEAT_X = { left: -14, mid: 0, right: 14 };

// 敵人類型（小兵）
// hp 血量、value 擊殺獎勵倍率（相對下注）、weapon 武器、faction 陣營配色
export const ENEMY_TYPES = [
  {
    id: 'sword', label: '刀兵', weapon: 'sword',
    hp: 3, value: 2, speed: 1.6, scale: 1.0,
    faction: 0xb23a2a, // 紅（蜀）
  },
  {
    id: 'spear', label: '槍兵', weapon: 'spear',
    hp: 5, value: 4, speed: 1.3, scale: 1.05,
    faction: 0x2a5bb2, // 藍（魏）
  },
  {
    id: 'archer', label: '弓兵', weapon: 'bow',
    hp: 4, value: 6, speed: 1.9, scale: 0.95,
    faction: 0x2a9a5b, // 綠（吳）
  },
  {
    id: 'shield', label: '盾兵', weapon: 'sword',
    hp: 8, value: 8, speed: 1.0, scale: 1.15,
    faction: 0x8a6a2a, // 金
  },
];

// 場景（關卡）設定 ------------------------------------------------
// 每個場景一位「鎮守 Boss」：同時間場上只會有一位，在關前徘徊搦戰、
// 定時喊出經典台詞；被斬殺後隔一段時間再度出關。
// 未來新增場景時在 SCENES 加一筆，並切換 CURRENT_SCENE 即可。
export const SCENES = [
  {
    id: 'hulao',
    name: '虎牢關',
    env: 'hulao',                 // 3D 環境（見 scene.js buildEnvironment）
    subtitle: '十八路諸侯討董',
    story: '十八路諸侯會盟討董，兵臨虎牢關。守關大將華雄連斬聯軍數員上將，正在關前耀武搦戰！',
    // 官銜菁英小兵：新生小兵有機率隨機掛上一個官銜（頭上顯示稱號牌）。
    // 同一時間場上每個官銜至多一人；持有者陣亡或離場，官銜隨之消失，
    // 冷卻一段時間後才會再出現在新的小兵身上。
    eliteTitles: ['西涼槍兵隊長', '西涼騎兵校尉', '飛熊軍百夫長', '弓騎統領'],
    eliteChance: 0.22,                       // 每隻新生小兵獲得官銜的機率（尚有空缺時）
    eliteCooldownMin: 8, eliteCooldownMax: 16, // 官銜消失後重新出現的冷卻秒數區間
    // 小兵閒談（西涼守軍視角，同時最多兩位小兵說話）：
    // 恐懼聯軍、崇拜華雄、軍令、餉銀牢騷……讓戰場有故事感。
    soldierQuotes: [
      '十八路諸侯都殺來了……',
      '有華雄將軍在，怕什麼！',
      '相國有令：死守虎牢關！',
      '那員猛將朝這邊來了！快躲！',
      '弟兄們結陣！別被他衝散了！',
      '我、我的腿一直在發抖……',
      '殺一個夠本，殺兩個賺一個！',
      '軍令如山：後退者，斬！',
      '洛陽的娘還等我回家……',
      '這關一破，天下就要變了。',
      '呂將軍的赤兔馬日行千里！',
      '昨夜風聲鶴唳，沒敢合眼…',
      '對面旌旗遮天，數不清人馬…',
      '聽說劉關張三兄弟也來了…',
    ],
    // 一問一答的對話組：兩位相鄰小兵接話（前句先說，後句約一秒後回）
    soldierDialogues: [
      ['喂，聽說潘鳳也被斬了？', '噓——小聲點，別亂軍心！'],
      ['對面那員猛將好生威風…', '別看了！他方才連斬十人！'],
      ['十八路諸侯到底多少人馬？', '數不清……旌旗遮天蔽日啊。'],
      ['華雄將軍今日又搦戰了？', '連斬兩將，威風得緊！'],
      ['餉銀到底何時發啊？', '打贏這仗再說吧……唉。'],
      ['我想家了……', '撐著點，活著回西涼。'],
      ['聽說溫侯就在關上觀戰？', '有呂將軍在，此關必不破！'],
      ['昨夜營裡又跑了十幾個…', '莫學他們，抓回來要斬首的！'],
    ],
    // 被玩家武將點擊鎖定時的驚呼（讓玩家有「他們怕我」的帶入感）
    targetedQuotes: [
      '他盯上我了！救命——',
      '別過來……別過來啊！',
      '我還不想死啊——',
      '來吧！西涼男兒不懼你！',
      '盾牌手！快攔住他！',
      '娘啊——他衝過來了！',
    ],
    // 華雄被斬時的驚呼（立即取代場上閒談，同樣至多兩位）
    bossDownQuotes: [
      '不好了！華雄將軍被斬了！',
      '將軍敗了？快、快報洛陽！',
      '天吶，誰還能擋住聯軍…',
      '完了完了，關要守不住了…',
      '溫侯！只能靠溫侯出馬了！',
    ],
    boss: {
      id: 'huaxiong',
      name: '華雄',
      title: '董卓帳前都督',
      hp: 60, value: 100, speed: 1.1, scale: 1.85, faction: 0x8a2430,
      firstSpawn: 8,                  // 開局後幾秒首次出關
      respawnMin: 16, respawnMax: 28, // 被斬殺後重新出關的秒數區間
      quotes: {
        entry: '吾乃董卓帳前都督華雄是也！關東鼠輩，誰敢與吾決一死戰？',
        taunts: [
          '插標賣首之徒，也敢犯吾虎牢關！',
          '祖茂、俞涉、潘鳳，皆斬於吾刀下！',
          '十八路諸侯，竟無一人敢出戰乎？哈哈哈！',
          '斬汝等首級，如探囊取物耳！',
          '再遣無名下將，也是白白送死！',
        ],
        death: '不好……此人刀快……酒、酒尚溫乎……',
      },
    },
  },
  {
    id: 'chibi',
    name: '赤壁',
    env: 'chibi',
    subtitle: '火燒連環船',
    story: '孫劉聯軍屯兵南岸，曹操八十萬大軍鐵索連舟。黃蓋詐降、東風將起，一場大火即將改寫天下！',
    eliteTitles: ['夏侯惇', '張遼', '許褚', '曹仁'],
    eliteChance: 0.22,
    eliteCooldownMin: 8, eliteCooldownMax: 16,
    // 曹軍北方兵在連環船上：暈船、疫病、思鄉、對火攻的不安……
    soldierQuotes: [
      '暈船暈得三天沒吃飯了…',
      '鐵索連舟後總算不晃了！',
      '這江上濕冷，甲都生鏽了…',
      '軍中又有弟兄染了疫病…',
      '破了江東，就能回北方了！',
      '夜裡江面全是霧，滲人啊…',
      '丞相昨夜橫槊賦詩，真雄主也！',
      '聽說黃蓋要率船來降？',
      '那員猛將跳上船了！攔住他！',
      '穩住！我們可是青州精兵！',
      '我不會水啊……落江就完了…',
      '月明星稀，烏鵲南飛……',
    ],
    soldierDialogues: [
      ['船都綁死了，逃得掉嗎？', '龐先生的連環計，錯不了！'],
      ['若東吳用火攻怎麼辦？', '隆冬哪來東風？多慮了。'],
      ['聽說周瑜才三十出頭？', '江東子弟，不可小覷…'],
      ['黃蓋的降船今夜就到？', '盯緊江面，別出岔子！'],
      ['我想回許昌了……', '打完這仗就回家了。'],
      ['昨夜夢見大江著火…', '呸呸呸！休得胡言！'],
    ],
    targetedQuotes: [
      '敵將殺上船來了！',
      '護住甲板！別讓他過來！',
      '我是北方人，不會水啊——',
      '虎衛軍！快來救我！',
      '來吧！曹家軍不懼你！',
      '他、他朝我來了——',
    ],
    bossDownQuotes: [
      '丞相中箭了？快護駕！',
      '快護丞相走華容道——',
      '火起了！連環船逃不掉啊！',
      '大勢已去，各自逃命吧！',
      '張遼將軍！斷後啊——',
    ],
    boss: {
      id: 'caocao',
      name: '曹操',
      title: '漢丞相',
      hp: 80, value: 150, speed: 1.0, scale: 1.8, faction: 0x2a5bb2,
      firstSpawn: 8,
      respawnMin: 16, respawnMax: 28,
      quotes: {
        entry: '吾乃漢相曹操！今統雄師百萬，願與將軍會獵於江東！',
        taunts: [
          '鐵索連舟，如履平地，哈哈哈！',
          '周郎孺子，安敢擋吾大軍？',
          '對酒當歌，人生幾何！',
          '待破江東，銅雀台上鎖二喬！',
          '隆冬臘月，安得東南風耶？',
        ],
        death: '東南風？！奉孝若在，不使孤至此……撤往華容道！',
      },
    },
  },
];

// 每個房間隨機輪替一個場景（換房 = 換戰場、換 Boss、換台詞）
for (const room of ROOMS) {
  room.sceneId = SCENES[(Math.random() * SCENES.length) | 0].id;
}

export function sceneById(id) {
  return SCENES.find((s) => s.id === id) || SCENES[0];
}

// 戰場範圍（世界座標）
export const FIELD = {
  minX: -22, maxX: 22,
  minZ: -20, maxZ: -1,   // 敵人活動的縱深區域（越遠 z 越小）
  turretZ: 6,            // 玩家砲台位置
};

// 假造的跑馬燈中獎訊息
export const MARQUEE_NAMES = [
  '常山趙子龍', '燕人張翼德', '臥龍先生', '江東小霸王',
  '虎痴許褚', '錦帆賊甘寧', '美髯公', '飛將軍',
];
export const MARQUEE_TARGETS = ['呂布', '關羽', '張飛', '顏良', '文醜', '華雄'];
